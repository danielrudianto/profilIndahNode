...
const pelangganBase = z.object({
  name: teksWajib(ErrorList["Customer name is required"]).max(
    PANJANG_PELANGGAN.name,
    ErrorList["Customer name too long"]
  ),
  pic: teksWajib(ErrorList["Customer PIC is required"]).max(
  ...
  address: teksWajib(ErrorList["Customer address is required"]).max(
...
/** POST /customer */
export const buatPelangganSchema = pelangganBase;
 ...
 */
export const ubahPelangganSchema = z.object({
  id: z
    .any()
    .refine(
...
export const paramPelangganSchema = z.object({
...
export const buatPerusahaanSchema = perusahaanBase;
export const ubahPerusahaanSchema = perusahaanBase;

export const paramPerusahaanSchema = z.object({
  id: intDariTeks(ErrorList["Parameter error"], 1),
});
...
const metodeBase = z.object({
  ...
  description: teksWajib(ErrorList["Parameter error"]),
});

export const buatMetodeSchema = metodeBase;

/** PUT /payment-method — urutan lama: id, name, description. */
export const ubahMetodeSchema = z.object({
  id: z
    .any()
    .refine(
      (nilai) => nilai !== undefined && nilai !== null && String(nilai) !== "",
...
export const paramMetodeSchema = z.object({
  id: intDariTeks(ErrorList["Parameter error"], 1),
});
 ...
 */
export const hapusMetodeSchema = z.object({
  id: z
    .any()
