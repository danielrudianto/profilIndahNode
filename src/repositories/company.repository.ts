...
export class CompanyRepository {
  ...
  async create(data: ICompany): Promise<CompanyModel> {
    try {
  ...
  async update(data: ICompany): Promise<CompanyModel> {
    try {
  ...
  async delete(id: number, userID: number) {
    ...
    return new CompanyModel({
      ...
      deleted_at: new Date(),
      user_company_deleted_byTouser: new UserViewModel({
  ...
  async fetchAutocomplete(keyword: string): Promise<CompanyModel[]> {
  ...
  async fetchAll(): Promise<CompanyModel[]> {
    try {
      const companies = await this.prisma.company.findMany({
        orderBy: {
          name: "asc",
  ...
  async fetchByID(id: number): Promise<CompanyModel | null> {
    const company = await this.prisma.$queryRaw<any[]>`
    SELECT company.id, company.name, company.address, 
    company.npwp, company.created_by, company.created_at, 
    company.is_delete, 
    IF(COALESCE(companyCount.count, 0) = 0,"1", "0") AS can_delete
