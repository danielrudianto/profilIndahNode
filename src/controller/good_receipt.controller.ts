...
class GoodReceiptController {
    ...
    static create = (req: Request, res: Response) => {
        ...
        const company_validation = CompanyModel.getById(company_id);
        const supplier_validation = SupplierModel.getById(supplier_id);

        const transaction_validation = new QueryTransactionHelper();
        transaction_validation.create([
            company_validation,
            supplier_validation
        ]).then(validation => {
            if(validation[0] == null || validation[1] == null || validation[0].is_delete || validation[1].is_delete){
            ...
            good_receipt.create().then(good_receipt_result => {
                ...
                transaction.create(transactions).then(result => {
                    ...
                    const insert_item = GoodReceiptModel.insertItems(good_receipt_items);
                    const purchase_document = new PurchaseDocumentModel(purchase_invoice_name, date, discount, good_receipt_result.id, req.body.userId);
                    const insert_purchase_document = purchase_document.create();

                    transaction.create([
                        insert_item,
                        insert_purchase_document
