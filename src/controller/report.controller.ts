...
class ReportController {
  ...
  static fetchInventoryReport = (req: Request, res: Response) => {
    const report = parseInt(req.params.report);
    if (report == 0) {
      StockValueHelper.fetchValue(new Date()).then(result => {
        return res.status(200).send
      })
    } else if (report == 1) {
    } else if (report == 2) {
