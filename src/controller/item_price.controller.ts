...
class ItemPriceController {
    static create = (req: Request, res: Response) => {
        ...
        const item_price = new ItemPriceModel(price, discount, discount_project, item_id, req.body.userId);
        const item_price_create = item_price.create();
        const item_price_update = ItemPriceModel.deleteById(item_id, req.body.userId);
        ...
        ItemModel.fetchByReferences(references).then(items => {
            if(items.length != count){
                ...
                references.forEach((reference, index) => {
                    ...
                    const item_price = new ItemPriceModel(price_object[index].price, price_object[index].discount, price_object[index].discount_project, items.filter(x => x.reference == reference)[0].id, req.body.userId, effective_date);
                    transactions.push(item_price.create());
                });

                const transaction = new QueryTransactionHelper();
                ItemPriceModel.deleteByIds(item_ids, req.body.userId).then(() => {
    ...
    static fetchAll = (req: Request, res: Response) => {
        ...
        ItemModel.fetchAll(date).then(items => {
            items.forEach(item => {
                result.push({
    ...
    static fetch = (req: Request, res: Response) => {
        ...
        ItemPriceModel.fetch(keyword, date, offset, limit).then(result => {
