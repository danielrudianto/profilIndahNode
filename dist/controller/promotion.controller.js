    ...
    constructor(promotionRepository, productRepository) {
        ...
        this.fetchByID = async (req, res) => {
            ...
            try {
                ...
                if (!promotion) {
                    return res.status(404).send(error_list_1.default["Promotion not found"]);
                }
        ...
        this.downloadSalesResultByID = async (req, res) => {
            var _a, _b;
            const id = Number(req.params.id);
            const promotion = await this.promotionRepository.fetchByID(id);
            if (!promotion) {
            ...
            const result = await this.promotionRepository.fetchSalesResult(productID, promotion.startDate, promotion.endDate);
            return res.status(200).send({
                data: result,
            });
        };
        this.downloadPurchaseResultByID = async (req, res) => {
            var _a, _b;
            const id = Number(req.params.id);
            const promotion = await this.promotionRepository.fetchByID(id);
            if (!promotion) {
            ...
            const result = await this.promotionRepository.fetchPurchaseReport(productID, promotion.supplier_id, promotion.startDate, promotion.endDate);
            return res.status(200).send({
                data: result,
            });
        };
