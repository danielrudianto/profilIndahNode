    ...
    item_model_1.ItemModel.fetchById(id).then((item) => {
        if (item == null || item.length == 0) {
            ...
            Promise.all([
                ...
                item_purchase_price_model_1.default.fetchByItemID(id),
            ]).then((result) => {
                return res.status(200).send({
                    id: item[0].id,
                ...
                return res.status(200).send({
                    data: result.hits.map((x) => {
                        ...
                        if (itemIndex != -1) {
                            ...
                            const draftSumIndex = items[1].findIndex((z) => z.item_id == x.id);
                            return {
                                id: x.id,
                                reference: x.reference,
                                description: x.description,
                                item_type: {
                                    name: (_b = item[itemIndex].item_type) === null || _b === void 0 ? void 0 : _b.name,
                                },
                                item_brand: {
