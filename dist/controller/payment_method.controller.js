...
PaymentMethodController.fetch = (req, res) => {
    ...
    PaymentMethodModel.fetch(keyword, offset, limit)
        .then((result) => {
        BillModel.countByPaymentMethodIds(result[0].map((x) => {
            return x.id;
        }))
            .then((count) => {
...
PaymentMethodController.submit = (req, res) => {
    ...
    const paymentMethod = new PaymentMethodModel(name, description, req.body.userId);
    paymentMethod
        .create()
        .then((result) => {
        LogHelper.log(result.created_at, "info", `${result.user.name} berhasil menambahkan metode pembayaran dengan nama ${result.name} (ID: ${result.id}).`, "Payment Method - Create", req.body.userId);
        const socket = new SocketHelper("createPaymentMethod", Object.assign(Object.assign({}, result), { can_delete: true }));
        socket.create();
        return res.status(201).send(result);
    })
        .catch((error) => {
        LogHelper.log(new Date(), "error", error, "Payment Method - Create", req.body.userId);
        return res.status(500).send(error);
    });
};
...
PaymentMethodController.fetchById = (req, res) => {
    ...
    PaymentMethodModel.fetchById(id)
        ...
        .catch((error) => {
        LogHelper.log(new Date(), "error", error, "Payment Method - Fetch by ID", req.body.userId);
        return res.status(500).send(error);
    });
};
PaymentMethodController.update = (req, res) => {
    ...
    PaymentMethodModel.fetchById(id)
        ...
        if (payment_method[0] == null || payment_method[0].is_delete) {
            ...
            const paymentMethod = new PaymentMethodModel(name, description, req.body.userId, id);
            paymentMethod
                .update()
                .then((result) => {
                ...
                .catch((error) => {
                LogHelper.log(new Date(), "error", error, "Payment Method - Update", req.body.userId);
                return res.status(500).send(error);
            });
        }
    })
