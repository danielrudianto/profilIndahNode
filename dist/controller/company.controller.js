...
CompanyController.fetch = (req, res) => {
    ...
    CompanyModel.fetch(keyword, offset, limit)
        .then((result) => {
        GoodReceiptModel.countByCompanyIds(result[0].map((x) => {
            return x.id;
        }))
            .then((counts) => {
            ...
            .catch((error) => {
            LogHelper.log(new Date(), "error", error, "Company - Fetch", req.body.userId);
        });
    })
        .catch((error) => {
        LogHelper.log(new Date(), "error", error, "Company - Fetch", req.body.userId);
...
CompanyController.delete = (req, res) => {
    ...
    CompanyModel.fetchById(id)
        ...
        CompanyModel.delete(id, req.body.userId)
            .then((company_result) => {
            var _a;
            (_a = CompanyModel.count()) === null || _a === void 0 ? void 0 : _a.then((company_count) => {
                var _a;
                LogHelper.log(company_result.deleted_at, "info", `${(_a = company_result.user_company_deleted_byTouser) === null || _a === void 0 ? void 0 : _a.name} deleted company with the name ${company_result.name} (ID: ${company_result.id})`, "Company - Delete", req.body.userId);
            ...
            .catch((error) => {
            LogHelper.log(new Date(), "error", `${error}`, `Item - Delete`, req.body.userId);
            return res.status(500).send(error);
        });
    })
        .catch((error) => {
...
CompanyController.update = (req, res) => {
    ...
    CompanyModel.getByCodeName(code_name)
        ...
        const company = new CompanyModel(name, address, npwp, req.body.userId, code_name, id);
        company
            .update()
            .then((company_result) => {
            var _a;
            LogHelper.log(company_result.updated_at, "info", `${(_a = company_result.user_company_updated_byTouser) === null || _a === void 0 ? void 0 : _a.name} updated company with the name ${company_result.name} (ID: ${company_result}`, "Company - Update", req.body.userId);
            io.emit("updateCompany", company_result);
            return res.status(201).send(company_result);
        })
            .catch((error) => {
            LogHelper.log(new Date(), "error", error, "Company - Update", req.body.userId);
            return res.status(500).send(error);
        });
    })
        .catch((error) => {
...
CompanyController.create = (req, res) => {
    ...
    const company = new CompanyModel(name, address, npwp, req.body.userId, code_name);
    company
        .create()
        .then((result) => {
        LogHelper.log(result.created_at, "info", `${result.user.name} created company with the name ${result.name} (ID: ${result.id})`, "Company - Create", req.body.userId);
        io.emit("createCompany", Object.assign(Object.assign({}, result), { can_delete: true }));
        return res.status(201).send(result);
    })
        .catch((error) => {
        LogHelper.log(new Date(), "error", error, "Company - Create", req.body.userId);
        return res.status(500).send(error);
    });
};
CompanyController.fetchById = (req, res) => {
    ...
    transaction
        ...
        .catch((error) => {
        LogHelper.log(new Date(), "error", error, "Company - Fetch by ID", req.body.userId);
        return res.status(500).send(error);
    });
};
