...
const meili_helper_1 = require("./meili.helper");
...
class SearchHelper {
    constructor(productRepository) {
        this.fillProducts = async (req, res) => {
...
_a = SearchHelper;
SearchHelper.createIndex = async (req, res) => {
    await meili_helper_1.meili.deleteIndexIfExists("item");
    await meili_helper_1.meili.deleteIndexIfExists("customer");
    await meili_helper_1.meili.deleteIndexIfExists("package");
    await meili_helper_1.meili.deleteIndexIfExists("product");
    await meili_helper_1.meili.deleteIndexIfExists("products");
    // await meili.createIndex("item", {
    //   primaryKey: "id",
    // });
    // await meili.createIndex("customer");
    // await meili.createIndex("package");
    await meili_helper_1.meili.index("product").updateSettings({
...
SearchHelper.getTasks = async (req, res) => {
    return res.status(200).send(await meili_helper_1.meili.getTask(15));
};
/**
 ...
 * @param res
 */
SearchHelper.syncMasterData = async (req, res) => {
 ...
 * @param res
 */
SearchHelper.syncProductIn = async (req, res) => {
 ...
 * @param res
 */
SearchHelper.syncProductOutCalculation = async (req, res) => {
    const stockOuts = await database_helper_1.prisma.$queryRawUnsafe(`
        SELECT * FROM 
        (
          # Adjustment case
          SELECT 
 ...
 * @param res
 */
SearchHelper.syncProductOut = async (req, res) => {
    const products = await mongo_product_model_1.mongoProductModel.find({});
    const stockCards = await database_helper_1.prisma.$queryRawUnsafe(`
      SELECT * FROM (
        # Good receipt
    ...
    await mongo_stock_card_model_1.mongoStockCardModel.insertMany(stockCardItems);
    return res.status(200).send({
 ...
 * Sync product card
 */
SearchHelper.arrangeStockCard = async (req, res) => {
    mongo_product_model_1.mongoProductModel.find({}).then(async (products) => {
