...
class ItemModel {
    ...
    static fetchStockData(item_id, start = null, end = null) {
        ...
        WHERE stock_card.item_id = ${item_id}`),
            ...
            return prisma.$transaction([
                prisma.$queryRawUnsafe(`SELECT COALESCE(billTable.name, goodReceiptTable.name, adjustmentTable.name) AS name, COALESCE(billTable.date, goodReceiptTable.date, adjustmentTable.date) AS date, stock_card_act.quantity, stock_card_act.stock
          FROM stock_card_act
          LEFT JOIN (
