...
class BillCodeModel {
     ...
     */
    static fetchByID(id) {
        return app_1.prisma.bill_code.findUnique({
            where: {
                id: id,
            },
            select: {
    ...
    static search(customers, items, date, keyword, page, mode) {
      ...
      LEFT JOIN customer ON bill_code.customer_id = customer.id`;
        ...
        return app_1.prisma.$transaction([
            app_1.prisma.$queryRawUnsafe(`${query} ${conditionalQueries} ORDER BY bill_code.date DESC LIMIT 10 OFFSET ${(page - 1) * 10}`),
    ...
    static fetchArchive(data) {
        switch (data.mode) {
            ...
            FROM bill_code
            LEFT JOIN customer ON bill_code.customer_id = customer.id
            WHERE YEAR(bill_code.date) = ${data.year} AND MONTH(bill_code.date) = ${data.month + 1}
            ${data.keyword == null
                        ? ""
          ...
          `),
            ...
            LEFT JOIN customer ON bill_code.customer_id = customer.id
            WHERE YEAR(bill_code.date) = ${data.year} AND MONTH(bill_code.date) = ${data.month + 1} AND bill_code.is_delete = 1
          ${data.keyword == null
                        ? ""
                        : `AND (bill_code.name LIKE '%${data.keyword}%' 
                OR COALESCE(customer.name, 'Retail customer') 
          ...
          `),
            ...
            SELECT COUNT(bill_code.id) AS count FROM bill_code
            LEFT JOIN customer ON bill_code.customer_id = customer.id
            WHERE YEAR(bill_code.date) = ${data.year} AND MONTH(bill_code.date) = ${data.month + 1} AND bill_code.is_delete = 0
          ${data.keyword == null
                        ? ""
                        : `AND (bill_code.name LIKE '%${data.keyword}%' 
              OR COALESCE(customer.name, 'Retail customer') 
    ...
    static fetchArchiveV2(data) {
      ...
      FROM (
        ...
        FROM bill_code
        LEFT JOIN customer ON bill_code.customer_id = customer.id
        WHERE YEAR(bill_code.date) = ${data.year} AND MONTH(bill_code.date) = ${data.month}
    ...
    static fetchChartItems(monthly, limit, offset) {
        ...
        if (monthly) {
            ...
            return app_1.prisma.$transaction([
                app_1.prisma.$queryRawUnsafe(`SELECT year, month, (delivery + value - discount + service) AS value, diff FROM (
          ...
          GROUP BY YEAR(bill_code.date), MONTH(bill_code.date)) AS bill_a`),
            ...
            return app_1.prisma.$queryRawUnsafe(`SELECT diff, (delivery + value - discount + service) AS value FROM (
    ...
    static countByCustomerIds(customer_ids) {
        return app_1.prisma.bill_code.groupBy({
            by: ["customer_id"],
            where: {
                customer_id: {
                    in: customer_ids,
    ...
    countByDate(date) {
        return app_1.prisma.bill_code.count({
            where: {
                date: {
    ...
    static fetchRecentSales() {
        ...
        WHERE bill_code.is_delete = 0
        AND bill_code.date BETWEEN '${todayDate.toISOString().split("T")[0]}' AND '${todayDate.toISOString().split("T")[0]}'
      `),
            app_1.prisma.$queryRawUnsafe(`
        SELECT SUM(b.value) - bill_code.discount + bill_code.delivery + bill_code.service AS value
        FROM bill_code
        ...
        ON bill_code.id = b.bill_code_id
        WHERE bill_code.is_delete = 0
        AND bill_code.date BETWEEN '${yesterdayDate.toISOString().split("T")[0]}' AND '${yesterdayDate.toISOString().split("T")[0]}'
      `),
        ]);
    }
    /**
    ...
    static fetchBillIDByCustomerIDV2(customerID, page) {
        return app_1.prisma.$transaction([
            app_1.prisma.bill_code.findMany({
                where: {
                    is_confirm: true,
                    is_delete: false,
                    is_paid: false,
