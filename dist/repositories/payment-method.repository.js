...
const payment_method_model_1 = require("../model/payment-method.model");
class PaymentMethodRepository {
    ...
    async create(data) {
        try {
            ...
            return new payment_method_model_1.PaymentMethodModel(Object.assign(Object.assign({}, result), { can_delete: true }));
        }
        catch (error) {
            throw error;
        }
    ...
    async update(data) {
        try {
            ...
            return new payment_method_model_1.PaymentMethodModel(Object.assign(Object.assign({}, result), { can_delete: data.can_delete }));
        }
        catch (error) {
            throw error;
        }
    ...
    `;
        ...
        return {
            data: result.map((item) => new payment_method_model_1.PaymentMethodModel({
                id: item.id,
                name: item.name,
    ...
    async fetchAutocomplete(keyword) {
        try {
            ...
            return result.map((item) => new payment_method_model_1.PaymentMethodModel({
                id: item.id,
                name: item.name,
                description: item.description,
    ...
    async fetchByID(id) {
        try {
            ...
            return new payment_method_model_1.PaymentMethodModel({
                id: result.id,
                name: result.name,
