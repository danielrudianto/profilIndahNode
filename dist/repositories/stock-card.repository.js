...
exports.StockCardRepository = void 0;
...
class StockCardRepository {
    ...
    async fetchByID(id) {
        ...
        return stock_card_model_1.StockCardModel.fromMap(result);
    }
    ...
    async fetchPrevious(data) {
        ...
        return result == null ? null : stock_card_model_1.StockCardModel.fromMap(result);
    }
    async fetch(data) {
        ...
        return stock_card_model_1.StockCardModel.fromMap(entry);
    }
    ...
    async reorder() {
        try {
            ...
            console.info(`[info]: Found ${productIDs.length} products that needs to be reorder`);
            for (let i = 0; i < productIDs.length; i++) {
                console.info(`[info]: Start reordering ${i + 1}/${productIDs.length} product stock card`);
                ...
                for (let j = 0; j < stockCards.length; j++) {
                    ...
                    updateQuery.push(this.prisma.stock_card.update({
                        where: {
                            id: stockCards[j].id,
                        },
                        data: {
                ...
                console.info(`[info]: Done reordering ${i + 1}/${productIDs.length} product stock card`);
            }
    ...
    async checkExistingByProductID(productID) {
        try {
...
exports.StockCardRepository = StockCardRepository;
//# sourceMappingURL=stock-card.repository.js.map