...
class ProductRepository {
        ...
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                if (item.product_unit_id != null) {
                    updateData.push(this.prisma.product_unit.update({
                        data: {
                    ...
                    updateData.push(this.prisma.product.update({
                        data: {
                            sales_price: item.price,
                            sales_discount: item.discount,
                        },
    ...
    async toggleActive(productID, currentStatus) {
        try {
            ...
            console.error(`[error]: Error while toggling product active status: ${error}`);
            throw new Error("Internal server error");
        }
    }
        ...
        try {
          ...
          `),
            ...
            return {
                ...
                brands: brands.map((x) => {
                    return product_brand_model_1.ProductBrandViewModel.fromMap(x);
                }),
                types: types.map((x) => {
                    return product_type_model_1.ProductTypeViewModel.fromMap(x);
                }),
            };
        }
        catch (error) {
    ...
    async fetchAll() {
        try {
            ...
            return results.map((item) => product_model_1.ProductModel.fromMap(item));
        }
        catch (error) {
            throw error;
        }
